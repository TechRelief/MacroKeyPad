...
void resetKbLeds()
{
  digitalWrite(SCR_LK_LED, HIGH);
  digitalWrite(NUM_LK_LED, HIGH);
  digitalWrite(CAP_LK_LED, HIGH);
}

void updateLeds()
{
  digitalWrite(CAP_LK_LED, bCapsLock ? LOW : HIGH);
  digitalWrite(NUM_LK_LED, bNumLock ? LOW : HIGH);
  // digitalWrite(SCR_LK_LED, bScrollLock ? LOW : HIGH);
}
...
// Used when sending a raw report not needing a leading "NO_KEY_PRESSED" report to clear out any previous key
...
// Used if any previous key report may have to be cleared before sending the new report.
